...
/**
 * Controlador principal de la aplicación NanoFiles.
 * Implementa la lógica del autómata de estados del cliente,
 * procesando los comandos del usuario y coordinando las
 * operaciones con el directorio y otros peers de la red P2P.
 * 
 * @author Redes
 */
public class NFController {
	...
	private boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_FILELIST: {
			commandAllowed = (currentState == SERVING || currentState == ONLINE);
			break;
		}
		case NFCommands.COM_SERVE: {
	...
	private void updateCurrentState(boolean success) {
		...
		case NFCommands.COM_DOWNLOAD: {
            if(currentState == ONLINE) {
				currentState = DOWNLOADING; // Cambia a DOWNLOADING al descargar
                currentState = ONLINE;  // Vuelve a ONLINE después de descargar
            } else if(currentState == SERVING) {
				currentState = DOWNLOADING; // Cambia a DOWNLOADING al descargar
                currentState = SERVING; // Mantiene SERVING si estaba sirviendo
            }
            break;
        }
		default:
